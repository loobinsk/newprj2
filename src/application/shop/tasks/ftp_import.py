...
def import_catalog_from_ftp():
    ...
    print('Starting import from ftp {}'.format(start_time))
        ...
        try:
            ...
            for line in f.readlines():
                try:
                    ...
                    category_1 = Category.objects.filter(name__iexact=cat_1_name, parent__isnull=True).first()
                    category_2 = None
                    if cat_2_name:
                        category_2 = Category.objects.filter(name__iexact=cat_2_name, parent=category_1).first()

                    if cat_3_name and category_2:
                        category_3 = Category.objects.filter(name__iexact=cat_3_name, parent=category_2).first()
                    ...
                    with transaction.atomic():
                        ...
                        good = Good.objects.filter(vendor_code=vendor_code).first()
                        if not good:
                            good = Good()
    ...
    print('Import from ftp is finished and took {} seconds'.format((datetime.now() - start_time).seconds))
    return 'Ok'
